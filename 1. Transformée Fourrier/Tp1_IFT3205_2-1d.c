...
void supressHarm(float** m, int length, int width) {
  for(int x=length/2-30; x<=length/2+30;  x++) {
    for(int y=width/2-30; y<=width/2+30; y++) {
      m[x][y] = 0.0;
    }
  }
}

void deleteFarHarm(float** m, int length, int width) {
  for(int x=0; x<length; x++) {
    for(int y=0; y<width; y++) {
      if (sqrt((x - length/2) * (x - length/2) + (y - width/2) * (y - width/2)) > 7) {
        m[x][y]=0.0;
      }
    }
  }
...
int main(int argc,char **argv)
 {
  ...
  deleteOnYFreqAxis(MatriceImgI, length, width, 10);
  deleteOnYFreqAxis(MatriceImgR, length, width, 10);
