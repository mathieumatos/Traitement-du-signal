...
void zigzagWannabeDebruitage(float** img, int length, int width, int coef, int n) {

    int bloc_i, bloc_j, i, j, dist_i, dist_j;

    float** res = fmatrix_allocate_2d(length, width);
    float** imageBloc = fmatrix_allocate_2d(n, n);
    ...
    for(dist_i = 0; dist_i < n; dist_i++){
        for(dist_j = 0; dist_j < n; dist_j++) {
            for(bloc_i = 0; bloc_i < length/n; bloc_i++){
                for(bloc_j = 0; bloc_j < width/n; bloc_j++) {

                    for(i = 0; i < n; i++){
                        for(j = 0; j < n; j++) {
                            imageBloc[i][j] = img[ (dist_i + n*bloc_i + i) % length ][ (dist_j + n*bloc_j + j) % width ];
                        }
                    }
            
                    ddct8x8s(-5, imageBloc);
                    seuilOpp(imageBloc, coef, n);
                    ddct8x8s(38, imageBloc);
            
                    for(i = 0; i < n; i++){
                        for(j = 0; j < n; j++) {
                            res[(dist_i + n*bloc_i + i) % length][(dist_j + n*bloc_j + j) % width] += imageBloc[i][j];
                        }
                    }
                }
            }
...
int main(int argc,char** argv)
{
    ...
    for(int n = 0; n <= 90; n++) {
        ...
        zigzagWannabeDebruitage(debruitage, length, width, n, 8);
    ...
    copy_matrix(debruitage, ImgDegraded, length, width);
    zigzagWannabeDebruitage(debruitage, length, width, best, 8);
