...
float carree_de_la_diff(float** m1, float** m2, int length, int width) {
  float total = 0.0;
  for (int i=0; i < length; i++) {
    for (int j=0; j < width; j++) {
...
int main(int argc,char **argv)
 {
  ...
  float** mat_img = LoadImagePgm(NAME_IMG_IN,&lgth,&wdth);
  
  degradation(mat_img, mat_rest, length, width, psf, size, variance_noise);
  ...
  for(k=0; k<nbiter; k++) {
    ...
    for(i=0; i<length; i++) {
      for(j=0; j<width; j++) {
        float denominator = temp_m[i][j] + (variance_noise/(length*width)) / m_img_m[i][j];
        temp2_r[i][j] = temp_r[i][j] / denominator;
        temp2_i[i][j] = -temp_i[i][j] / denominator;
      }
    }

    for(i=0;i<length;i++) {
      for(j=0;j<width;j++) {
        mat_rest[i][j] = m_img_r[i][j];
        m_img_i[i][j] = 0;
      }
    }
    ...
    float num = carree_de_la_diff(mat_img, m_img_r, length, width);
    float den = carree_de_la_diff(mat_img, mat_rest, length, width);
